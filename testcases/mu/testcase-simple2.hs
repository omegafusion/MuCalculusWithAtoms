...
main = 
    let a = constant 0
        ...
        myFormulaExpected = Left $ Mu vx ([0], NL.singleton ([], Disjunction (Predicate p0) (Diamond (Variable vx))))
        -- is a state with Pred d reachable?
        myFormula2 = parser "M[ mu v1 . p3 | <>v1 ]"
        myFormula2Expected = Left $ Mu vy ([1], NL.singleton ([], Disjunction (Predicate p3) (Diamond (Variable vy))))

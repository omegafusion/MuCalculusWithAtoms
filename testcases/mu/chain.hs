...
main = let phi = parser "M[ |_a . nu v0_a { b . |_c . p0_c & (&_d/=c . ~p0_d) & <>(p0_b & (&_d/=b . ~p0_d) & <>v0_c) } ]"
       ...
       in do print $ check model1 phi `NL.eq` NL.singleton (State 0 [NL.constant 0]);
             print $ check model2 phi `NL.eq` NL.map (\a -> State 0 [a]) NL.atoms;
             print $ check model3 phi `NL.eq` (NL.fromList $ map (\i -> State i []) [0,2,4,6,8,10]);
             print $ check model4 phi `NL.eq` (NL.fromList $ map (\i -> State i []) [0,2,4,6,8,10]);